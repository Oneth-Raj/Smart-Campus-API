...
Step 2: Match {sensorId}/readings
        → sensorReadings() method
        → Returns new SensorReadingResource("CO2-001")

Step 3: Look for @GET method on returned resource
        → SensorReadingResource.getReadings()
...
readings.add(new SensorReading(...)); // ← Not safe under concurrent access
readings.get(0);                      // ← Another thread might be modifying
...
✓ **Clean Architecture**: Separation of concerns via sub-resource locators
✓ **Concurrency**: Thread-safe data structures for high throughput
✓ **Error Handling**: Semantic HTTP status codes + security-aware responses
✓ **Validation**: Foreign key constraints prevent data orphans
✓ **Extensibility**: Query parameter filtering scales to multiple criteria
✓ **Observability**: Request/response logging for debugging
